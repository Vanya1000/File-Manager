...
const compressAction = async ([pathSource, pathDist]) => {
  try {
    ...
    const { ext: extDist, root: rootDist, name: nameDist } = parse(pathDistAbsolute);
    ...
    if (!extDist) {
      ...
      extDist === '.br' ? pathDistFile = join(rootDist, `${nameDist}${extSource}${'.br'}`) : pathDistFile = join(`${pathDistAbsolute}.br`);
    }
    
    console.log(pathDistFile);
